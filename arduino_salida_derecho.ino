/*
 * Smart Portium — Arduino UNO #2
 * PORTÓN SALIDA (Derecho) — MOT-110
 *
 * Conexión ESP32:
 *   Pin 10 (RX) ← ESP32 GPIO25 (TX)
 *   Pin 11 (TX) → ESP32 GPIO26 (RX)
 *
 * Microswitches:
 *   Pin 2 = SW_CERRADO (INPUT_PULLUP, NO)
 *   Pin 3 = SW_ABIERTO  (INPUT_PULLUP, NO)
 *
 * Motor MOT-110 (derecho — GIRO INVERTIDO):
 *   write(0)   = abrir
 *   write(180) = cerrar
 *   write(90)  = detener
 */
...
// GIRO INVERTIDO para portón derecho
void motorAbrir()   { motor.write(0);   }
void motorCerrar()  { motor.write(180); }
...
void setup() {
  ...
  Serial.println("Arduino Salida (Derecho) listo");
}
